...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		public static object ReadClass (
			...
			List<float> floats, 
			List<object> references)  //if type is not existing - using the recommended one if it's defined
		{
			...
			if (objType == null) { Debug.Log("Calss: Could not load " + header + " as this type does not exists anymore."); return null; }
			/*{ 
				if (recommendedType == null) { Debug.Log("Could not load " + header + " as this type does not exists anymore"); return null; }
				else 
				{
					Debug.Log("Could not load " + header + " as this type does not exists anymore. Trying to replace it with " + recommendedType);
					objType = recommendedType;
				}
			}*/
			...
			else 
			{
				...
				catch(Exception e) { Debug.LogError("Error deserializing " + objType + " " + obj + ": " + e.ToString()); }
			}
			...
			while (true)
			{
				...
				//trying to find a new type using the value name
				if (val.type == null) 
				{
					Debug.Log("Value: Could not load " + typeName + " as this type does not exists anymore."); continue; 

					//FieldInfo field = objType.GetField(val.name);
					//val.type = field.FieldType;
					//Debug.Log("Could not load " + typeName + " as this type does not exists anymore. Trying to replace it with " + val.type); //continue; 
				} 
			...
			else 
			{
				...
				foreach (FieldInfo field in objType.UsableFields(includeStatic:true))  
				{
					...
					for (int i=0; i<valuesCount; i++)
						if (values[i].name == fieldName  &&  (values[i].type==fieldType || values[i].type.IsSubclassOf(fieldType) )) //values[i].type == fieldType)//  &&  (values[i].obj==null || values[i].obj.GetType() == fieldType))
